...
if ($site_config["MEMBERSONLY"]){
	...
	if($CURUSER["view_torrents"]=="no")
		show_error_msg("Error","You do not have permission to view nfo's",1);
}
...
if (!$id)
	show_error_msg("ID not found", "You can't view, if you don't tell me what you want!",1);

stdhead("View NFO");

$query = mysql_query("SELECT name, nfo FROM torrents WHERE id=$id") or sqlerr();
$res = mysql_fetch_assoc($query);

if ($res["nfo"] != "yes")
  show_error_msg("Not Found", "This torrent does not have a NFO");
...
if($res["nfo"] == "yes"){
    ...
    if ($nfo) {
		...
		if($CURUSER["edit_torrents"]=="yes")
            begin_frame("NFO File For: <a href=".$site_config["SITEURL"]."/torrents-details.php?id=$id>$shortname</a> - <a href=nfo-edit.php?id=$id>[Edit NFO]</a>");
        else
            begin_frame("NFO File For: $shortname<a name=nfo></a>");

		begin_table();
		print("<tr><td>\n");
		print("<textarea style=\"font-size:8pt;width:100%;height:100%;\" wrap=\"off\" rows=50 and cols=20 READONLY>".stripslashes($nfo)."</textarea>");
		end_table();
