...
if (!is_valid_id($id))
	show_error_msg("Error", "Torrent doesn't exist.",1);
...
if (isset($_COOKIE["TTrsreq$reseedid"])){ //check cookie for spam prevention
	...
	if (mysql_num_rows($res) < 1){
		show_error_msg("Error", "Torrent doesn't exist",0);
		stdfoot();
		die;
	}
	...
	while ($srow = mysql_fetch_array($sres)){
		...
		$res =mysql_query("SELECT id, username FROM users WHERE id = ".$srow["userid"]." ");
